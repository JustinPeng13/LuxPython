...
def smart_transfer_to_nearby(game, team, unit_id, unit, target_type_restriction=None, **kwarg):
    ...
    nearby carts first, then any worker. Transfers the resource type in order of uranium, wood, coal (assume order of environment-friendliness). 
    Picks which cart/worker based on choosing a target that is most-full but able to take the most amount of resources.

    Underlying rationale is that we want to "build" with the most envi-friendly resources.
    ...
    env_policy_threshold =  {"uc": 0.7, "uw": 0.4, "wc": 0.2}

    if unit != None:

        # check for uranium -> wood -> coal
        tuw = (abs(unit.cargo['uranium']- unit.cargo['wood'])/max(unit.cargo['uranium'], unit.cargo['wood'])) if max(unit.cargo['uranium'], unit.cargo['wood']) > 0 else 0
        tuc = (abs(unit.cargo['uranium']- unit.cargo['coal'])/max(unit.cargo['uranium'], unit.cargo['coal'])) if max(unit.cargo['uranium'], unit.cargo['coal']) > 0 else 0
        twc = (abs(unit.cargo['wood']- unit.cargo['coal'])/max(unit.cargo['wood'], unit.cargo['coal'])) if max(unit.cargo['wood'], unit.cargo['coal']) > 0 else 0
        
        #best case: uranium more than both or diff. less than threshold
        if unit.cargo['uranium'] > unit.cargo['coal'] and unit.cargo['uranium'] > unit.cargo['wood'] or (tuw < env_policy_threshold["uw"] and tuc < env_policy_threshold["uc"]):
            resource_amount = unit.cargo['uranium']
            resource_type = Constants.RESOURCE_TYPES.URANIUM
        
        else:
            if unit.cargo['wood'] > unit.cargo['coal'] or twc < env_policy_threshold:
                #diff less than threshold
                resource_amount = unit.cargo['wood']
                resource_type = Constants.RESOURCE_TYPES.WOOD
            else:
                #worst case - coal
                resource_amount = unit.cargo['coal']
                resource_type = Constants.RESOURCE_TYPES.COAL
